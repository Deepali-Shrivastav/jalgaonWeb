        ...
        try {
          if (data.industry) {
            const relRes = await fetch(
              `${baseUrl}/api/v1/startups/?industry=${data.industry.slug}`,
            );
            if (relRes.ok) {
      ...
      <div className="max-w-[1200px] mx-auto px-4 md:px-6 lg:px-8">
        ...
        <nav className="flex items-center gap-2 text-xs md:text-sm font-semibold text-secondary mb-8 overflow-x-auto whitespace-nowrap pb-2 scrollbar-none">
          ...
          {startup.industry && (
            <>
              <Link
                href={`/startups?industry=${startup.industry.slug}`}
                className="hover:text-primary transition-colors"
              >
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  <h1 className="text-2xl sm:text-3xl font-black text-ink-deep tracking-tight">
                    {startup.name}
                  </h1>
                  {startup.is_verified && (
                    <span
                      className="bg-blue-50 text-blue-700 px-2.5 py-0.5 rounded-full text-[10px] font-bold flex items-center gap-1 border border-blue-100"
                ...
                <div className="flex flex-wrap gap-2 text-xs">
                  ...
                  <span
                    className={`px-2.5 py-1 rounded-md font-bold ${getStageBadgeClass(startup.stage)}`}
                  >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8 items-start">
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-3xl p-6 md:p-8 border border-outline-variant shadow-sm space-y-4">
              <h2 className="text-xl font-bold text-ink-deep border-b border-hairline-soft pb-3">
                About the Startup
              </h2>
              ...
              {startup.founders && startup.founders.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  {startup.founders.map((founder) => (
                    <div
                      key={founder.id}
                      className="bg-surface-container-low rounded-2xl p-5 border border-hairline-soft flex flex-col justify-between"
                    >
                      <div>
                        <div className="flex items-center gap-4 mb-4">
                          ...
                          <div>
                            <h4 className="font-bold text-ink-deep text-base leading-snug">
                              {founder.name}
                            </h4>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-3xl p-6 border border-outline-variant shadow-sm space-y-4">
              <h3 className="text-lg font-bold text-ink-deep border-b border-hairline-soft pb-2">
                Location & Contact
              </h3>
              ...
              <div className="space-y-3.5 text-sm text-on-surface-variant">
                ...
                {!startup.address && !startup.phone && !startup.email && (
                  <p className="text-secondary text-xs italic">
                    Contact details are private or unlisted.
                  </p>
                )}
              </div>
            </div>
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div className="space-y-1 overflow-hidden">
                          <h4 className="font-bold text-ink-deep text-sm truncate">
                            {rel.name}
                          </h4>
