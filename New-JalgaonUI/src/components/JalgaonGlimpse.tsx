...
export default function JalgaonGlimpse({ initialData }: JalgaonGlimpseProps) {
  ...
  return (
    <section id="jalgaon-glimpse" className="bg-surface-container-low py-6 sm:py-10 md:py-12" aria-labelledby="jalgaon-glimpse-heading">
      <div className="mx-auto max-w-container-max px-4 sm:px-6 md:px-xxl">
        {/* White Card Box Container */}
        <div className="bg-white border border-hairline-soft rounded-2xl sm:rounded-3xl p-5 sm:p-8 md:p-10 shadow-sm sm:shadow-md">
          {/* Section Header */}
          ...
          {/* Modern YouTube Cards Grid (1 video on mobile, 3 videos on desktop) */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-6">
            {displayVideos.slice(0, 3).map((video, idx) => (
              <div
                key={`${video.video_id}-${idx}`}
                className={`group flex-col cursor-pointer ${idx > 0 ? 'hidden md:flex' : 'flex'}`}
              >
                <Link href={`/jalgaon-glimpse/${video.video_id}`} className="block">
                  {/* 16:9 Thumbnail Card with Rectangular Rounded Corners */}
                  <div className="relative aspect-video w-full bg-slate-900 rounded-xl sm:rounded-2xl overflow-hidden mb-2.5 sm:mb-3 shadow-sm sm:shadow-md border border-hairline-soft">
                    <img
